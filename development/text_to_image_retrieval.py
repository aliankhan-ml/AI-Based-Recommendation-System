...
def text_to_image(text_query, model, image_embeddings, top_k=6):
    ...
    with torch.no_grad():
        similarity = (100.0 * image_embeddings @ text_features.T).squeeze().cpu().numpy()

    # Get top k results
    top_indices = np.argsort(similarity)[::-1][:top_k]

    print(f"\nTop {top_k} results for query: '{text_query}'")
...
if __name__ == "__main__":
    ...
    # Example query
    text_query = "pink and yellow flowers with green leaves"
    top_indices, similarity = text_to_image(text_query, model, image_embeddings)